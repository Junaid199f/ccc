...
class Population():
  def __init__(self,block_size,population_size,bounds):
    self.element = []
    ...
    self.individuals  = [ self.generate_individuals(self.block_size,bounds) for i in range(self.population_size)]
    self.individuals = asarray(self.individuals)
  ...
  def generate_individuals(self,block_size,bounds):
    individual = []
    for i in range(len(bounds)):
      if i % 2 == 0:
        individual.append(round(random.uniform(0, 0.99), 2))
      else:
        individual.append(int(random.choice(self.params_choices[str(i)])))
    return individual
